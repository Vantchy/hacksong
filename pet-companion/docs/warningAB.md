...
## 六、A 的 UI 决策通知（2026-09-02，请 B/C 知悉并配合）

### 背景

A 已完成全站 UI 去 emoji 改造：面板大框已取消、字体加粗可爱化、窗户加大偏右、底部栏按钮改为纯 CSS 图标式（无文字，仅图标 + 消耗标注）、属性标签纯文字（无圆点图案）、宠物特效（心形/Z字/闪电/气泡）全部改为纯 CSS 图形。新增/废弃 class 已登记到 samename.md「A 专属装饰 class」一节。

### 需 C 配合：清理 gameLogic.js 中的 message emoji

以下 message 会渲染到主站 UI 气泡，与全站无 emoji 风格冲突，请 C 删除消息中的 emoji 前缀：

| 位置（当前行号） | 当前文案 | 修改方式 |
|------|----------|----------|
| `actions` 各项 message（66-116 行） | '😊 暖暖的～' / '👋 它抬头看了你一眼' / '🍔 好吃～谢谢你！' / '🧼 好干净！' / '✋ 啪！配合完美！' / '💪 加油加油！' / '🥰 它蹭了蹭你的手' / '😴 充能完毕！' | 去掉 emoji + 空格前缀 |
| `getWarning`（372-375 行） | '🍔 我好饿……' / '🧹 该洗澡了……' / '😴 好累啊……' / '😢 陪我玩会儿吧……' | 去掉 emoji + 空格前缀 |
| `claimDailyLogin`（413 / 423 行） | '📝 先写下今天的学习目标吧！' / '🌅 写下目标，获得 X 专注星！' | 去掉 emoji + 空格前缀 |
| `claimDailyGoal`（447 行） | '🎯 完成今日目标，获得 3 专注星！' | 去掉 emoji + 空格前缀（本文档 B-5 的示例代码同步更新） |
| `leaveFocus`（504 行） | '✨ 专注时段结束，获得 X 专注星！' | 去掉 emoji + 空格前缀 |

**测试断言联动（重要）**：`tests/test_c_node.js:389` 与 `tests/test_c.html:434` 断言了 `'📝 先写下今天的学习目标吧！'`，C 修改 message 时必须同步修改这两处断言，否则测试会失败。测试工具自身的装饰符号（✓ ✗ 等，仅显示于测试页/控制台）不影响主站 UI，可不改。

### 废弃 class / 结构变更提醒（B）

- `.stat-dot` / `.dot-hunger` / `.dot-happiness` / `.dot-hygiene` 已从 HTML 删除，属性标签为纯文字，B 如有引用请移除
- `.pet-sick` 与 `catSick` 动画已从 pet.css 删除，B 不得再切换该 class
- mood 记录按钮已从 emoji（😊😐😫）改为文字（开心/一般/低落），`data-mood` 值不变，B 的事件绑定不受影响
- 底部栏按钮文字已移除（改为图标），B 绑定仍用 `data-action`，不受影响；悬停提示用按钮的 `title` 属性